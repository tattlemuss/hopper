...
extern bool calc_relative_address(const operand& op, uint32_t inst_address, uint32_t& target_address);

// Write out an instruction's opcode and operands to the file stream.
extern void print(const instruction& inst, const symbols& symbols, uint32_t inst_address, FILE* pFile);
