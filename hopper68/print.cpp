...
bool calc_relative_address(const hopper68::operand& op, uint32_t inst_address, uint32_t& target_address)
{
	if (op.type == hopper68::PC_DISP)
	{
		target_address = inst_address + op.pc_disp.inst_disp;
		return true;
	}
	else if (op.type == hopper68::PC_DISP_INDEX)
	{
		target_address = inst_address + op.pc_disp_index.inst_disp;
		return true;
	}
	else if (op.type == hopper68::RELATIVE_BRANCH)
	{
		target_address = inst_address + op.relative_branch.inst_disp;
		return true;
	}
	else if (op.type == hopper68::INDIRECT_POSTINDEXED || op.type == hopper68::INDIRECT_PREINDEXED ||
			 op.type == hopper68::MEMORY_INDIRECT || op.type == hopper68::NO_MEMORY_INDIRECT)
	{
		if (op.indirect_index_68020.base_register == hopper68::INDEX_REG_PC)
		{
...
// ----------------------------------------------------------------------------
static void print_index_indirect(const hopper68::index_indirect& ind, FILE* pFile)
{
	if (ind.index_reg == hopper68::INDEX_REG_NONE)
		return;
	fprintf(pFile, "%s.%s%s",
		hopper68::get_index_register_string(ind.index_reg),
		ind.is_long ? "l" : "w",
		hopper68::get_scale_shift_string(ind.scale_shift));
}
...
// ----------------------------------------------------------------------------
static void print_bitfield(const hopper68::bitfield& bf, FILE* pFile)
{
	fprintf(pFile, "{");
	...
	int brace_open, int brace_close, uint32_t inst_address, FILE* pFile)
{
	...
	for (int index = 0; index < 4; ++index)
	{
		if (ref.used[index])
		{
			...
			{
				case 0:
					if (ref.base_register == hopper68::IndexRegister::INDEX_REG_PC)
					{
					...
					break;
				case 1:
					fprintf(pFile, "%s", hopper68::get_index_register_string(ref.base_register)); break;
				case 2:
...
// ----------------------------------------------------------------------------
void print(const hopper68::operand& operand, const symbols& symbols, uint32_t inst_address, FILE* pFile)
{
	switch (operand.type)
	{
		...
		case hopper68::OpType::INDIRECT_INDEX:
			...
			fprintf(pFile, ")");
			return;
		case hopper68::OpType::ABSOLUTE_WORD:
			...
			return;
		case hopper68::OpType::ABSOLUTE_LONG:
		{
			symbol sym;
		...
		case hopper68::OpType::PC_DISP_INDEX:
		{
			symbol sym;
			uint32_t target_address;
			calc_relative_address(operand, inst_address, target_address);
			...
			if (find_symbol(symbols, target_address, sym))
			{
				fprintf(pFile, "%s(pc,%s.%s%s)",
						sym.label.c_str(),
						hopper68::get_index_register_string(operand.pc_disp_index.indirect_info.index_reg),
						operand.pc_disp_index.indirect_info.is_long ? "l" : "w",
						hopper68::get_scale_shift_string(operand.pc_disp_index.indirect_info.scale_shift));
			}
			else
			{
		...
		case hopper68::OpType::MOVEM_REG:
		{
			bool first = true;
			for (int i = 0; i < 16; ++i)
				if (operand.movem_reg.reg_mask & (1 << i))
				{
					if (!first)
						fprintf(pFile, "/");
					fprintf(pFile, "%s", hopper68::get_movem_reg_string(i));
					first = false;
				}
			return;
		}
		case hopper68::OpType::RELATIVE_BRANCH:
		{
			symbol sym;
			uint32_t target_address;
			calc_relative_address(operand, inst_address, target_address);
		...
		case hopper68::OpType::IMMEDIATE:
			...
			return;
		case hopper68::OpType::D_REGISTER_PAIR:
			fprintf(pFile, "d%u:d%u", operand.d_register_pair.dreg1, operand.d_register_pair.dreg2);
			return;
		case hopper68::OpType::INDIRECT_REGISTER_PAIR:
			fprintf(pFile, "(%s):(%s)",
				hopper68::get_index_register_string(operand.indirect_register_pair.reg1),
				hopper68::get_index_register_string(operand.indirect_register_pair.reg2));
			return;
		...
		case hopper68::OpType::CCR:
			fprintf(pFile, "ccr");
			return;
		case hopper68::OpType::CONTROL_REGISTER:
			fprintf(pFile, "%s", hopper68::get_control_register_string(operand.control_register.cr));
			return;
		default:
			fprintf(pFile, "???");
	}
...
// ----------------------------------------------------------------------------
void print(const hopper68::instruction& inst, const symbols& symbols, uint32_t inst_address, FILE* pFile)
{
	...
	fprintf(pFile, "%s", hopper68::get_opcode_string(inst.opcode));
	fprintf(pFile, "%s", hopper68::get_suffix_string(inst.suffix));

	if (inst.op0.type != hopper68::OpType::INVALID)
	{
		fprintf(pFile, " ");
	...
	if (inst.op1.type != hopper68::OpType::INVALID)
	{
		fprintf(pFile, ",");
	...
	if (inst.op2.type != hopper68::OpType::INVALID)
	{
		fprintf(pFile, ",");
